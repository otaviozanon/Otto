...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off("room:state", onUpdate);
    };
  }, []);
      ...
      <div className="w-full max-w-md space-y-8 animate-scale-in">
        ...
        <div className="space-y-2">
          ...
          {room.players.map((p, i) => (
            ...
            >
              <div
                className={`shrink-0 w-3 h-3 rounded-full ${p.connected ? "bg-accent-success shadow-[0_0_6px_rgba(34,197,94,0.4)]" : "bg-accent-warning"}`}
              />
        ...
        {isHost ? (
          <button
            onClick={() => {
              if (canStart) getSocket().emit("room:start");
            }}
            disabled={!canStart}
