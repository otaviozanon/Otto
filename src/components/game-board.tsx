...
export default function GameBoard() {
  ...
  useEffect(() => {
    ...
    if (isMyTurnNow) {
      ...
      if (bannerTimerRef.current) {
        ...
        bannerTimerRef.current = undefined;
      }
    }
  ...
  const playableCards = useMemo(
    () =>
      gameState.hand.map((card: CardType, i: number) => {
        if (!isMyTurn) return false;
        if (
          room?.stackChain &&
          (room.stackChain.type === "draw2" || room.stackChain.type === "wild4")
        ) {
          return card.type === room.stackChain.type;
        }
        if (card.type === "wild" || card.type === "wild4") return true;
        const top = gameState.currentCard;
        if ("color" in card && card.color === gameState.currentColor)
          return true;
        if (
          card.type !== "number" &&
          top.type !== "number" &&
  ...
  const handleDraw = useCallback(() => {
    getSocket().emit("game:draw-card");
    setSelectedIndex(null);
  }, []);
  const handlePlayDrawn = useCallback(
    () => getSocket().emit("game:play-drawn-card"),
    [],
  );
        ...
        {room?.stackChain && (
          ...
          >
            <div className="px-4 py-2 rounded-xl bg-brand/20 border border-brand/30 flex items-center gap-2 text-sm text-brand font-semibold pointer-events-auto">
              ...
              Empilhamento ativo:{" "}
              {STACK_LABELS[room.stackChain.type] || room.stackChain.type} ×
              {room.stackChain.count}
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      <div className="flex-1 flex flex-col items-center justify-center gap-6 p-4 pt-2 pb-0">
        <div className="flex items-center gap-12">
          <div className="flex flex-col items-center gap-1.5">
            <div className="relative w-[70px] h-[98px]">
              {Array.from({ length: Math.min(gameState.drawPileCount, 3) }).map(
                (_, i) => (
                  <div
                    key={i}
              ...
              <div
                className={`w-2.5 h-2.5 rounded-full shadow-[0_0_6px_currentColor] ${
                  gameState.currentColor === "red"
                    ? "bg-uno-red text-uno-red"
                    : gameState.currentColor === "blue"
                      ? "bg-uno-blue text-uno-blue"
                      : gameState.currentColor === "green"
                        ? "bg-uno-green text-uno-green"
          ...
          <div className="flex flex-col items-center gap-1.5">
            {isMyTurn ? (
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-black transition-colors duration-300 ${
                  timer <= 5
        ...
        <ActionButtons
          ...
          canStack={gameState.canStack}
          hasSelectedCard={selectedIndex != null}
          hasDrawn={hasDrawn}
          drawnPlayable={gameState.drawnCardPlayable}
          calledUno={gameState.calledUno}
          handSize={gameState.hand.length}
          isMyTurn={isMyTurn}
          onPlay={handlePlay}
          onDraw={handleDraw}
          onPlayDrawn={handlePlayDrawn}
          onPass={handlePass}
          onUno={handleUno}
        />
      </div>
      ...
      <AnimatePresence>
        {unoNotif && (
          <motion.div
            ...
            className="fixed top-[155px] left-1/2 -translate-x-1/2 z-35 pointer-events-none"
          >
      ...
      <AnimatePresence>
        {showTurnBanner && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="fixed top-[155px] left-1/2 -translate-x-1/2 z-30 pointer-events-none"
          >
            <div className="px-4 py-1 rounded-full bg-uno-yellow/15 border border-uno-yellow/30 backdrop-blur-sm">
              <span className="text-[11px] font-bold text-uno-yellow tracking-[0.2em] uppercase">
                ◆ Seu Turno
              </span>
            </div>
          </motion.div>
